...
class PointRestraint:
    ...
    def get_moment_sign(self):
        ...
        if self.location == self.x_values[0]:
            m_epsilon = self.get_moment_value(2 * self.toleranc)
...
class Beam:
    ...
    def __init__(self,
                 ...
                 start_support: PointRestraint,
                 end_support: PointRestraint,
        ...
        for bs in bound_restraints:
            ...
            for x_value in z:
                if start < x_value < end:
                    point_restraints.append(PointRestraint(None, x_value, restraint, x_values, m_values, m_func, middle_of_bounding_restraint=True))
            point_restraints.append(PointRestraint(None, start, restraint, x_values, m_values, m_func, start_of_bounding_restraint=True))
            point_restraints.append(PointRestraint(None, end, restraint, x_values, m_values, m_func, end_of_bounding_restraint=True))
...
def convert_beam_restraints_to_points(
    ...
    bounds_point_restraints = convert_bounding_restraints_to_point_restraints(
        [bounds], x_values, m_values
    )
    point_restraints = []
    for point in points:
        pr = PointRestraint(None, point[0], point[1], x_values, m_values)
...
def get_beam_point_restraints_with_respect_to_supports_and_remove_duplicates(
    ...
    if isinstance(start_support, (tuple, list)):
        start_support = PointRestraint('A', start_support[0], start_support[1], x_values, m_values)
    if isinstance(end_support, (tuple, list)):
        end_support = PointRestraint('B', end_support[0], end_support[1], x_values, m_values)
    if isinstance(point_restraints[0], (tuple, list)):
        point_restraints = [PointRestraint(None, p[0], p[1], x_values, m_values) for p in point_restraints]
...
def get_segments_of_frame(
    ...
    point_restraints, bounds_restraints = convert_beam_restraints_to_points(restraints, x_values, m_values)
